...
import datetime
from irasai.forms import utility_password_check
import secrets
...
from irasai.forms import RegistracijosForma, PaskyrosAtnaujinimoForma, SlaptazodzioAtnaujinimoForma, IrasasForm, PrisijungimoForma, UzklausosAtnaujinimoForma
...
def save_picture(picture):
    ...
    _, f_ext = os.path.splitext(picture.filename)
    picture_fn = random_hex + f_ext
    picture_dir = os.path.join(app.root_path, 'static', 'profilio_nuotraukos')
    ...
    try:
        i.save(picture_path, format='JPEG')
        print("Picture saved:", picture_path)
        return picture_fn
    except Exception as e:
        print("Error while saving picture:", e)
        return None
...
def new_record():
    ...
    if form.validate_on_submit():
        naujas_irasas = Irasas(irasaaaass=form.irasas.data, vartotojas_id=current_user.id, data=datetime.datetime.now())
...
def registruotis():
    ...
    if form.validate_on_submit():
        if utility_password_check(form.slaptazodis.data):
            ...
            flash("blogas slaptazodis zinok", 'danger')
        return redirect(url_for('index'))
